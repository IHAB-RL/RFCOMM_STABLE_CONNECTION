...
package com.example.ihabluetoothaudio.bluetotohspp.library;
...
public class BluetoothService {
    ...
    public synchronized void start(boolean isAndroid) {
        ...
        if (mSecureAcceptThread == null) {
            ...
            mSecureAcceptThread.setPriority(Thread.MAX_PRIORITY);
    ...
    public synchronized void connect(BluetoothDevice device) {
        ...
        mConnectThread.setPriority(Thread.MAX_PRIORITY);
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    arr_byte[idx++] = (byte)mmInStream.read();
                    if(idx == arr_byte.length) {
                        idx = 0;
                        //mHandler.obtainMessage(BluetoothState.MESSAGE_READ, arr_byte.length, -1, Arrays.copyOf(arr_byte, arr_byte.length)).sendToTarget();
                        mHandler.obtainMessage(BluetoothState.MESSAGE_READ, arr_byte.length, -1, Arrays.copyOf(arr_byte, arr_byte.length)).sendToTarget();
                    }
                } catch (IOException e) {
                    connectionLost();
